...
public class p139 {
    ...
    public boolean wordBreak(String s, List<String> wordDict) {
        ...
        //递推过程 dp[i]如果可以被拆分 那么可以分解成 前[0,k]个字符已经可以拆分 然后[k+1,i]对应的字符存在于字典中
        boolean dp[]=new boolean[s.length()+1];
        dp[0]=true;//dp[0]表示空字符串  边界

        //遍历字符串长度 当i=1的时候 表示s的中长度为1的时候 其是否可以被拆分的清空 dp[1]即前1个字符
        for (int i=1;i<=s.length();i++){
            ...
            for (int k=0;k<i;k++){
                ...
                //如果此时已经知道dp[i]=true 表示前i个字符构成的字符串可以在字典中找到拆分的结果 直接推出 判断下一个即dp[i+1]
                if (dp[i]==true)
                    break;
            }
        }
